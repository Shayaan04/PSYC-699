...
This Community Data Lab technical report responds to World Bank trade policy interests (via the Kansas Data Science Consortium) by asking whether trade association density (a proxy for organized industry capacity) is associated with newly introduced non-tariff measures (NTMs) in global panel data. NTMs include sanitary and technical regulations, licensing, and related instruments recorded in the World Integrated Trade Solution (WITS) alongside tariffs. The project merges WITS notifications, KDSC digitized association counts from the \textit{World Guide to Trade Associations}, and UNIDO INDSTAT-based synthetic industry controls into a country, year, and industry panel (73 countries, 137 industries, 34 years in the filtered extract).
...
Figure~\ref{fig:global_ntm} summarizes worldwide notification activity in the WITS NTM archive for 1991 to 2024. The left panel counts \emph{new} NTMs introduced each year (aggregated globally). The series is volatile in the 1990s and 2000s, rises through the 2010s, and shows very large spikes in the mid-2010s before falling back toward low levels in the most recent years. The right panel plots the implied \emph{cumulative} stock of NTMs over the same period: slow accumulation through the 2000s, a steep increase when the mid-decade spikes arrive, and a plateau thereafter. Together, the panels illustrate why the empirical work emphasizes flexible functional forms (log transforms) and year fixed effects: both the level and the volatility of recorded NTM activity shift over time, and tail years may also reflect reporting and database coverage in addition to true policy incidence.
...
\begin{figure}[htbp]
  ...
  \IfFileExists{fig_ntm_global_trends.png}{%
    ...
    \IfFileExists{figures/fig_ntm_global_trends.png}{%
      ...
      \end{minipage}}}}%
  \caption{Non-tariff measures over time (1991 to 2024). \emph{Left:} new NTMs introduced per year (global total). \emph{Right:} cumulative NTMs (global stock). Source: WITS NTM database.}
...
A central construction challenge is that WITS NTMs are recorded at the HS level while trade association and industry control series are organized using ISIC industry codes. Official WITS concordance tables map HS codes to ISIC Revision~4. Records are collapsed to a country, year, and industry panel. The key outcome used in the regressions is \texttt{new\_ntm\_count}: the number of new NTMs introduced in a given country, industry, and year.
...
The final analysis dataset is a country, year, and industry panel in ISIC Revision~4. Constructing it required reconciling three sources that do not arrive in a single native key: WITS NTMs are recorded at the HS product level with HS nomenclature vintages that differ across countries and years; the digitized trade association inventory is keyed to country and industry concepts aligned to ISIC; and UNIDO INDSTAT synthetic controls are reported at the country, industry, and year level in ISIC space.

The merge therefore proceeded in stages. First, raw WITS NTM records were aggregated to the country, year, and HS level (and related intermediate steps consistent with the upstream extract), then each HS series was mapped to ISIC Revision~4 using official WITS concordance tables. Because concordance weights and applicable HS revisions can differ by reporter, the HS-to-ISIC crosswalk was applied \emph{separately for each country}: for every country, HS codes were translated to ISIC Rev.~4 with that country's relevant nomenclature version, and the mapped measures were re-aggregated to country, year, and industry cells. This country-by-country crosswalk avoids imposing a single HS vintage where it would mis-classify another reporter's codes.
...
\begin{figure}[htbp]
  ...
  \caption{Average new NTMs per year by World Bank income group (1991 to 2024). Source: WITS; World Bank income classifications.}
  \label{fig:fig1}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Average new NTMs: high-income countries versus all other countries (1991 to 2024). Source: WITS.}
  \label{fig:fig2}
\end{figure}
...
Figures~\ref{fig:fig4} through~\ref{fig:fig6} display the estimated coefficient on $\ln(1+\mathrm{TA})$ with 95\% confidence intervals for Models~1 to~3.
...
Natural extensions include tracking associations over time if longitudinal data become available; incorporating import penetration and demand elasticities to move closer to structural tests in the spirit of \citet{grossmanHelpman1994} and related work such as \citet{mitraEtAl2002}; estimating models for specific NTM chapters (especially Types~A and~B); and using instrumental variables to tighten causal interpretation.
...
Calendar year of the observation, 1991 to 2024 in the filtered extract. Year fixed effects are absorbed in all regression models.

\noindent\textbf{\texttt{isic\_rev4}:}
Numeric International Standard Industrial Classification (ISIC) Revision~4 industry code for the country, year, and industry cell.
...
World Bank income classification for that country and year. Values are L (Low Income), LM (Lower-Middle Income), UM (Upper-Middle Income), or H (High Income), based on Gross National Income (GNI) per capita thresholds defined annually by the World Bank.
...
Main outcome variable. Count of new non-tariff measures (NTMs) introduced in a given country, industry, and year, drawn from the World Integrated Trade Solution (WITS) database maintained jointly by the World Bank and the United Nations Conference on Trade and Development (UNCTAD).
...
\noindent\textbf{\texttt{countA} through \texttt{countP}:}
...
Main explanatory variable. Number of trade associations operating in the country-industry, drawn from the digitized World Guide to Trade Associations (Zils 1999). Because the source is a single cross-sectional inventory, the count is repeated across all years in the panel for a given country-industry pair.
